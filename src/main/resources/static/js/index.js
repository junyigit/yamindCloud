	...
	if (getCookie('lang') == "zh-cn" ||getCookie('lang') == "zh_cn"){
		...
		console.log("进入index 菜单加载函数，进入中文cn-zh");
        //生成菜单
        var menuItem = Vue
            .extend({
                name : 'menu-item',
            ...
            console.log("进入index 菜单加载函数，进入英文en-us");
            //生成菜单
            var menuItem = Vue
                .extend({
                    name: 'menu-item',
                    ...
                    template: [
                        ...
                        '<menu-item :item="item" v-for="item in item.list"></menu-item>',
                        '</ul>',
                        '<a v-if="item.type === 1" :href="\'#\'+item.url"><i v-if="item.icon != null" :class="item.icon"></i><i v-else class="fa fa-circle-o"></i> {{item.enname}}</a>',
                        '</li>'].join('')
                });
