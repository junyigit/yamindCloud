...
public class ResourceController {
     ...
     * @return
     */
    @GetMapping(value = "imgs/{fileType}/{fileDate}/{fileName}", produces = {MediaType.IMAGE_JPEG_VALUE, MediaType.IMAGE_PNG_VALUE})
    public ResponseEntity returnIdeaImg(@PathVariable String fileType,@PathVariable String fileDate, @PathVariable String fileName, HttpServletRequest request) {
        try {
            // 获取请求文件后缀(.xxx)
            String requestURI = request.getRequestURI();
            String suffix = requestURI.substring(requestURI.indexOf("."), requestURI.length());
            // File.separator   linux \   windows // 或者 \
            String filePath = uploadPath +fileType + File.separator+fileDate + File.separator + fileName;
     ...
     * @return
     */
    @GetMapping(value = "img/{fileType}/{fileName}", produces = {MediaType.IMAGE_JPEG_VALUE, MediaType.IMAGE_PNG_VALUE})
    public ResponseEntity returnTypeImg(@PathVariable String fileType, @PathVariable String fileName, HttpServletRequest request) {
        try {
            // 获取请求文件后缀(.xxx)
            String requestURI = request.getRequestURI();
            String suffix = requestURI.substring(requestURI.indexOf("."), requestURI.length());
            // File.separator   linux \   windows // 或者 \
            String filePath = uploadPath +fileType + File.separator + fileName;
            return ResponseEntity.ok(resourceLoader.getResource("file:" + filePath));
        } catch (Exception e) {
            e.printStackTrace();
            return ResponseEntity.notFound().build();
